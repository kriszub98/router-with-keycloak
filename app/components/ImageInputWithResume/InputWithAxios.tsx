...
export const ImageUploadWholeFile: React.FC = () => {
  ...
  // upload pojedynczego, pełnego pliku jako base64 (1 request) – axios
  const uploadSingleFile = async (
    ...
    try {
      ...
      await axios.post("/api/upload-file", payload, {
        headers: {
        ...
        onUploadProgress: (event) => {
          if (event.total) {
            ...
            // gdy brak event.total możesz ewentualnie robić fake-progress
          }
        },
      });
      ...
      if (axios.isAxiosError(err)) {
        ...
        const text =
          ...
          axiosErr.message;
        msg = status
          ? `Błąd uploadu (status ${status}): ${text || "nieznany błąd"}`
          : `Błąd uploadu: ${text}`;
  ...
  return (
    <div className="max-w-xl mx-auto p-4 space-y-4">
      <h1 className="text-xl font-semibold">
        Upload zdjęć (pełne pliki, axios)
      </h1>
